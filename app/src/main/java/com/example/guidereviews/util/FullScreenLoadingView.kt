...
class FullScreenLoadingView(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
    ...
    init {
        ...
        attrs.let {
            val typedArray =
                context.obtainStyledAttributes(it, R.styleable.FullScreenLoadingView, 0, 0)
            val loaderSizeOrdinal = typedArray.getInt(
                R.styleable.FullScreenLoadingView_loader_size,
                0
            )
